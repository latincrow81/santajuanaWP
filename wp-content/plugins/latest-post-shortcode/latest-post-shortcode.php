...
class Latest_Post_Shortcode {
	 ...
	 * @return string
	 */
	public static function lps_pagination( $total = 1, $per_page = 10, $range = 4, $shortcode_id = '', $class = '' ) {
		wp_reset_postdata();
		$body     = '';
		$total    = (int) $total;
		...
		if ( $total > 1 ) {
			...
			$body .= '
			<ul class="latest-post-selection pages ' . esc_attr( $class ) . ' ' . esc_attr( $shortcode_id ) . '">
	...
	public static function latest_selected_content( $args ) {
		...
		if ( ! empty( $qargs['posts_per_page'] ) && ! empty( $args['showpages'] ) ) {
			...
			$pagination_html  = self::lps_pagination( intval( $found_posts ), ( ! empty( $qargs['posts_per_page'] ) ) ? $qargs['posts_per_page'] : 1, intval( $args['showpages'] ), $shortcode_id, $pagination_class );
			...
			if ( ! empty( $is_lps_ajax ) ) {
				...
				if ( in_array( 'ajax_pagination', $show_extra, true ) && ! empty( $args ) && is_array( $args ) ) {
					if ( version_compare( PHP_VERSION, '5.4', '<' ) ) {
						echo '<div id="' . esc_attr( $shortcode_id ) . '-wrap" data-args="' . esc_js( wp_json_encode( $args ) ) . '" data-current="' . get_the_ID() . '" class="lps-top-section-wrap">';
					} else {
						echo '<div id="' . esc_attr( $shortcode_id ) . '-wrap" data-args="' . esc_js( wp_json_encode( $args, JSON_UNESCAPED_UNICODE ) ) . '" data-current="' . get_the_ID() . '" class="lps-top-section-wrap">';
					}
				} else {
		...
		if ( ! empty( $posts ) ) {
			...
			foreach ( $posts as $post ) {
				...
				if ( $use_custom_markup ) {
					...
					// Tile text markup.
					$text = '';
					if ( in_array( 'excerpt', $extra_display, true ) || in_array( 'content', $extra_display, true )
						|| in_array( 'content-small', $extra_display, true )
						|| in_array( 'excerpt-small', $extra_display, true ) ) {
						if ( in_array( 'excerpt', $extra_display, true ) ) {
	...
	public static function latest_selected_content_slider( $posts, $args ) {
		...
		$imgsize  = ( empty( $args['image'] ) ) ? 'thumbnail' : $args['image'];
